...
const FarsanScreen = () => {
  ...
  const [quantities, setQuantities] = useState(
    farsanVariants.reduce((acc, item) => {
      acc[item.id] = 1;
      return acc;
    }, {})
  );
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: scale(30) }}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.title}>Farsan / फरसान</Text>
        </View>

        {/* Farsan List */}
        {farsanVariants.map(variant => {
          const qty = quantities[variant.id];
          ...
          return (
            <View key={variant.id} style={styles.variantCard}>
              ...
              <View style={styles.textContainer}>
                ...
                <Text style={styles.availabilityText}>Available in: {variant.availability}</Text>
