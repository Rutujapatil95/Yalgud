...
const MawaCakeScreen = () => {
  const navigation = useNavigation();

  const [quantities, setQuantities] = useState(
    mawaVariants.reduce((acc, item) => {
      acc[item.id] = 1;
      return acc;
    }, {})
  );
  ...
  return (
    <SafeAreaView style={styles.container}>
      <StatusBar backgroundColor="#eab308" barStyle="dark-content" />
      {/* Header */}
      <View style={styles.header}>
        <TouchableOpacity onPress={() => navigation.goBack()} style={styles.backButton}>
          <Text style={styles.backArrow}>←</Text>
        </TouchableOpacity>
        <Text style={styles.title}>Mawa Cakes / मावा केक</Text>
      </View>

      <ScrollView contentContainerStyle={{ paddingBottom: scale(30) }}>
        {mawaVariants.map(variant => {
          const qty = quantities[variant.id];
          const totalPrice = variant.price * qty;
          const totalMrp = variant.mrp * qty;
          const profit = totalMrp - totalPrice;
