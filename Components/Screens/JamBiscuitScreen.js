...
const jambiscuitVariants = [
  {
    id: '1',
    name: 'Classic Jambiscuit',
    price: 15,
    mrp: 20,
...
const JamBiscuitScreen = () => {
  const navigation = useNavigation();

  const [quantities, setQuantities] = useState(
    jambiscuitVariants.reduce((acc, item) => {
      acc[item.id] = 1;
      return acc;
    }, {})
  );
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: scale(30) }}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.title}>Jambiscuits</Text>
        </View>

        {/* Variants List */}
        {jambiscuitVariants.map(variant => {
          const qty = quantities[variant.id];
...
export default JamBiscuitScreen;
