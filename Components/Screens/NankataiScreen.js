...
const nankataiVariants = [
  {
    id: '1',
    name: 'Classic Nankatai',
    price: 40,
    mrp: 50,
...
const NankataiScreen = () => {
  const navigation = useNavigation();

  const [quantities, setQuantities] = useState(
    nankataiVariants.reduce((acc, item) => {
      acc[item.id] = 1;
      return acc;
    }, {})
  );
  ...
  return (
    <SafeAreaView style={styles.container}>
      <StatusBar backgroundColor="#FAD7A0" barStyle="dark-content" />
      <ScrollView contentContainerStyle={{ paddingBottom: scale(30) }}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.title}>Nankatai</Text>
        </View>

        {/* Variants */}
        {nankataiVariants.map(variant => {
          const qty = quantities[variant.id];
