...
const creamVariants = [
  {
    ...
    price: 22,
    mrp: 28,
    image: require('../Images/cream99.jpg'),
  },
  {
    id: '2',
...
const CreamBiscuitScreen = () => {
  const navigation = useNavigation();

  const [quantities, setQuantities] = useState(
    creamVariants.reduce((acc, item) => {
      acc[item.id] = 1;
      return acc;
    }, {})
  );
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Cream Biscuits</Text>
      </View>

      <ScrollView contentContainerStyle={{ paddingBottom: scale(30) }}>
        {creamVariants.map(variant => {
          const qty = quantities[variant.id];
...
export default CreamBiscuitScreen;
