...
const faraliVariants = [
  {
    id: '1',
    name: 'Classic Farali Biscuit',
    price: 18,
    mrp: 22,
...
const FaraliBiscuitScreen = () => {
  const navigation = useNavigation();

  const [quantities, setQuantities] = useState(
    faraliVariants.reduce((acc, item) => {
      acc[item.id] = 1;
      return acc;
    }, {})
  );
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: scale(30) }}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.title}>Farali Biscuits</Text>
        </View>

        {/* Variants List */}
        {faraliVariants.map(variant => {
          const qty = quantities[variant.id];
...
export default FaraliBiscuitScreen;
