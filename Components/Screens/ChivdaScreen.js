...
const ChivdaScreen = () => {
  ...
  const [quantities, setQuantities] = useState(
    chivdaVariants.reduce((acc, item) => {
      acc[item.id] = 1;
      return acc;
    }, {})
  );
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: scale(30) }}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.title}>Chivda / चिवडा</Text>
        </View>

        {/* Variants List */}
        {chivdaVariants.map(variant => {
